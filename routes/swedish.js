    ...
    return await request(`http://ipinfo.io/${ip}`, { json: true }, async(err, _, body) => {
        ...
        return await sheetClient.authorize()
            .then(async (tokens) => {
                ...
                return await googleSheetsService.spreadsheets.values.append(appendOptions)
                    .then((result) => {
                        ...
                        console.log("success")
                        return res.sendFile(path.join(__dirname, '../views', 'swedish.html'));
                    })
                    .catch((err) => {
                        console.log(err)
                        appendOptions.resource.values = [];
            ...
            .catch((error) => {
                ...
                res.sendFile(path.join(__dirname, '../views', 'swedish.html'));
            })
    });
});
      ...
      .then(async (tokens) => {
          ...
          return await googleSheetsService.spreadsheets.values.append(appendOptions)
              .then((result) => {
                  appendOptions.resource.values = [];
                  console.log("success")
                  return res.json({message: "Tack. Dina uppgifter har skickats framgångsrikt. Vi skriver till dig inom kort. Gud välsigne dig."});
              })
              .catch((err) => {
                  console.log(err)
                  appendOptions.resource.values = [];
