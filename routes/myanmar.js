    ...
    return await request(`http://ipinfo.io/${ip}`, { json: true }, async(err, _, body) => {
        ...
        return await sheetClient.authorize()
            .then(async (tokens) => {
                ...
                return await googleSheetsService.spreadsheets.values.append(appendOptions)
                    .then((result) => {
                        ...
                        console.log("success")
                        return res.sendFile(path.join(__dirname, '../views', 'myanmar.html'));
                    })
                    .catch((err) => {
                        console.log(err)
                        appendOptions.resource.values = [];
            ...
            .catch((error) => {
                ...
                res.sendFile(path.join(__dirname, '../views', 'myanmar.html'));
            })
    });
});
      ...
      .then(async (tokens) => {
          ...
          return await googleSheetsService.spreadsheets.values.append(appendOptions)
              .then((result) => {
                  appendOptions.resource.values = [];
                  console.log("success")
                  return res.json({message: "ကျေးဇူးတင်ပါတယ်။ သငျသညျအသေးစိတျကိုအောင်မြင်စွာတင်သွင်းပြီးလျှင် ငါတို့သည်သင်တို့မကြာမီသင်ရေးပါ။ ဘုရားသခင်သင့်ကိုကောင်းချီးပေးပါစေ။"});
              })
              .catch((err) => {
                  console.log(err)
                  appendOptions.resource.values = [];
