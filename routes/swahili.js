    ...
    return await request(`http://ipinfo.io/${ip}`, { json: true }, async(err, _, body) => {
        ...
        return await sheetClient.authorize()
            .then(async (tokens) => {
                ...
                return await googleSheetsService.spreadsheets.values.append(appendOptions)
                    .then((result) => {
                        ...
                        console.log("success")
                        return res.sendFile(path.join(__dirname, '../views', 'swahili.html'));
                    })
                    .catch((err) => {
                        console.log(err)
                        appendOptions.resource.values = [];
            ...
            .catch((error) => {
                ...
                res.sendFile(path.join(__dirname, '../views', 'swahili.html'));
            })
    });
});
      ...
      .then(async (tokens) => {
          ...
          return await googleSheetsService.spreadsheets.values.append(appendOptions)
              .then((result) => {
                  appendOptions.resource.values = [];
                  console.log("success")
                  return res.json({message: "Asante. Maelezo yako yamewasilishwa kwa mafanikio. Tunakuandikia hivi karibuni. Mungu akubariki."});
              })
              .catch((err) => {
                  console.log(err)
                  appendOptions.resource.values = [];
