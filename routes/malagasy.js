    ...
    return await request(`http://ipinfo.io/${ip}`, { json: true }, async(err, _, body) => {
        ...
        return await sheetClient.authorize()
            .then(async (tokens) => {
                ...
                return await googleSheetsService.spreadsheets.values.append(appendOptions)
                    .then((result) => {
                        ...
                        console.log("success")
                        return res.sendFile(path.join(__dirname, '../views', 'malagasy.html'));
                    })
                    .catch((err) => {
                        console.log(err)
                        appendOptions.resource.values = [];
            ...
            .catch((error) => {
                ...
                res.sendFile(path.join(__dirname, '../views', 'malagasy.html'));
            })
    });
});
      ...
      .then(async (tokens) => {
          ...
          return await googleSheetsService.spreadsheets.values.append(appendOptions)
              .then((result) => {
                  appendOptions.resource.values = [];
                  console.log("success")
                  return res.json({message: "Misaotra anao. Nahomby ny antsipiriany nataonao. Manoratra aminao vetivety izahay. Hotahian'andriamanitra anie ianao."});
              })
              .catch((err) => {
                  console.log(err)
                  appendOptions.resource.values = [];
