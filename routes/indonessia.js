      ...
      .then(async (tokens) => {
          ...
          return await googleSheetsService.spreadsheets.values.append(appendOptions)
              .then((result) => {
                  appendOptions.resource.values = [];
                  console.log("success")
                  return res.json({message: "Terima kasih. Detail Anda telah berhasil dikirim. Kami akan segera menyurati Anda. Tuhan memberkati Anda."});
              })
              .catch((err) => {
                  console.log(err)
                  appendOptions.resource.values = [];
                  return res.json({message: "Ada masalah dengan operasi pengiriman. Coba lagi nanti."});
              })
      })
      .catch((error) => {
          console.log(error);
