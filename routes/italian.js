...
router.get('/', async (req, res, next) => {
  ...
  return await sheetClient.authorize()
      .then(async (tokens) => {
          ...
          return await googleSheetsService.spreadsheets.values.append(appendOptions)
              .then((result) => {
                  ...
                  console.log("success")
                  return res.sendFile(path.join(__dirname, '../views', 'italian.html'));
              })
              .catch((err) => {
                  console.log(err)
                  appendOptions.resource.values = [];
      ...
      .catch((error) => {
          ...
          res.sendFile(path.join(__dirname, '../views', 'italian.html'));
      })
});
      ...
      .then(async (tokens) => {
          ...
          return await googleSheetsService.spreadsheets.values.append(appendOptions)
              .then((result) => {
                  appendOptions.resource.values = [];
                  console.log("success")
                  return res.json({message: "Grazie. I tuoi dettagli sono stati inviati con successo. Ti scriviamo a breve. Dio ti benedica."});
              })
              .catch((err) => {
                  console.log(err)
                  appendOptions.resource.values = [];
                  return res.json({message: "Si è verificato un problema con l'operazione di invio. Riprovare più tardi."});
              })
      })
      .catch((error) => {
          console.log(error);
